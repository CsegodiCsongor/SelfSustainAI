        ...
        public static void DrawMap()
        {
            ...
            for (int i = 0; i < MapSize; i++)
            {
                for (int j = 0; j < MapSize; j++)
                {
                    ...
                    {
                        grp.DrawRectangle(new Pen(Color.Red, 2f), Map[i, j].Top.Y - TopY, Map[i, j].Top.X - TopX, Map[i, j].Size, Map[i, j].Size);
                        if (Visible[i, j])
                        {
                            grp.DrawString(Map[i, j].ContaintID.ToString(), new Font("Arial", StringSize), new SolidBrush(Color.Red), Map[i, j].Top.Y - TopY, Map[i, j].Top.X - TopX);
                            if (Map[i, j].ContaintID != 0)
                            {
                                grp.DrawString(Map[i, j].ResourceAmmount.ToString(), new Font("Arial", StringSize), new SolidBrush(Color.Green), Map[i, j].Top.Y - TopY, Map[i, j].Top.X - TopX + StringSize+1);
                        ...
                        {
                            grp.FillRectangle(new SolidBrush(Color.Aqua), Map[i, j].Top.Y - TopY, Map[i, j].Top.X - TopX, Map[i, j].Size, Map[i, j].Size);
                        }
                        else
                        {
                            grp.DrawString(Map[i, j].ContaintID.ToString(), new Font("Arial", StringSize), new SolidBrush(Color.Red), Map[i, j].Top.Y - TopY, Map[i, j].Top.X - TopX);
